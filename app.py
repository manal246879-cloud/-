import streamlit as st
import google.generativeai as genai
from pypdf import PdfReader

# --- إعدادات الهوية البصرية (نفس ستايل جامعة نورة) ---
st.set_page_config(page_title="فزعة، تسولفها", page_icon="🌸", layout="centered")

custom_css = """
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Tajawal:wght@400;700&display=swap');
    
    html, body, [class*="css"] {
        font-family: 'Tajawal', sans-serif;
        direction: rtl;
        text-align: right;
    }
    .stButton>button {
        width: 100%;
        border-radius: 25px;
        height: 3em;
        background-color: #8A1538;
        color: white;
        border: none;
        font-weight: bold;
        transition: 0.3s;
    }
    .stButton>button:hover {
        background-color: #FCE4EC;
        color: #8A1538;
        border: 1px solid #8A1538;
    }
    .main {
        background-color: #FFFFFF;
    }
    h1 { color: #8A1538; }
    .stAlert { border-radius: 15px; }
    </style>
"""
st.markdown(custom_css, unsafe_allow_html=True)

# --- إعدادات API (حط مفتاحك هنا) ---
# genai.configure(api_key="YOUR_GEMINI_API_KEY")

# --- واجهة المستخدم ---
st.image("https://upload.wikimedia.org/wikipedia/ar/thumb/0/00/PNU_Logo.svg/1200px-PNU_Logo.svg.png", width=100) # شعار تقريبي
st.title("🌸 فزعة، تسولفها")
st.subheader("من تعقيد أكاديمي… إلى جلسة سوالف")

uploaded_file = st.file_uploader("ارفعي ملف المحاضرة (PDF)", type="pdf")

if uploaded_file is not None:
    # استخراج النص من الـ PDF
    reader = PdfReader(uploaded_file)
    full_text = ""
    for page in reader.pages:
        full_text += page.extract_text()
    
    st.success("تم رفع الملف بنجاح! اختاري الطريقة اللي تناسبك:")

    # الأزرار الثلاثة (بدون اختيار تلقائي)
    col1, col2, col3 = st.columns(3)
    
    with col1:
        if st.button("🇸🇦 سولفها بالعربي"):
            mode = "najdi"
            prompt = f"حول هذا النص الأكاديمي إلى شرح مفصل وعميق بلهجة نجدية بيضاء (سوالف)، بأسلوب داعم وأنثوي. لا تلخص، اشرح كل التفاصيل. حافظ على المصطلحات العلمية. النص: {full_text}"
            st.session_state['prompt'] = prompt

    with col2:
        if st.button("🇺🇸➡️🇸🇦 عربناها لك"):
            mode = "translate_najdi"
            prompt = f"ترجم النص التالي للعربي واشرحه بلهجة نجدية سوالف. حافظ على المصطلحات الإنجليزية المهمة كما هي بين القوسين. اشرح بعمق ولا تلخص. النص: {full_text}"
            st.session_state['prompt'] = prompt

    with col3:
        if st.button("🇬🇧 English to English"):
            mode = "english"
            prompt = f"Transform this academic text into a deep-dive conversational English explanation. Keep it friendly but intellectually rich. Do not summarize. Original text: {full_text}"
            st.session_state['prompt'] = prompt

    # --- معالجة الذكاء الاصطناعي ---
    if 'prompt' in st.session_state:
        with st.spinner("جاري تحويل المحتوى إلى سوالف ممتعة... ✨"):
            # هنا يتم استدعاء Gemini (تعطيل الاستدعاء الحقيقي لحين وضع المفتاح)
            # model = genai.GenerativeModel('gemini-1.5-pro')
            # response = model.generate_content(st.session_state['prompt'])
            # result_text = response.text
            
            result_text = "هنا سيظهر الشرح العميق بأسلوب السوالف بعد ربط الـ API الخاص بـ Gemini..." 
            
            st.markdown("---")
            st.markdown(f"### 📖 الشرح المفصل")
            st.write(result_text)
            
            # محاكي الصوت (TTS)
            st.audio("https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3") # تجريبي
            st.download_button("تحميل الشرح كملف نصي", result_text)
            st.button("تحميل الملف الصوتي MP3")

else:
    st.info("بانتظار ملفك الأكاديمي لنبدأ السوالف..")
