...
if uploaded_file:
    ...
    if full_text.strip():
        ...
        if col1.button("🇸🇦 سولفها بالعربي"):
            final_prompt = f"اشرحي المحتوى التالي بلهجة نجدية سوالف بنات عفوية وممتعة جداً بين (سارة ونورة): {full_text[:8000]}"
        if col2.button("🇺🇸➡️🇸🇦 عربناها لك"):
            final_prompt = f"النص التالي بالإنجليزية، ترجميه واشرحيه بلهجة نجدية سوالف بين بنتين (سارة ونورة) مع تبسيط المصطلحات: {full_text[:8000]}"
        if col3.button("🇬🇧 English"):
            final_prompt = f"Explain this academic text in a friendly conversational English dialogue between two girls (Sarah and Nora): {full_text[:8000]}"

        if final_prompt:
            with st.spinner("سارة ونورة يجهزون القهوة وبيسولفون لك... ☕"):
                sys_msg = "You are an expert at turning academic text into natural conversations between two girls, Sarah and Nora. Format: Sarah: [text] Nora: [text]. Max 3-4 exchanges."
                
                script = ""
                # نظام المحاولات المتعددة لتجنب الـ 404
                ...
                if not script:
                    st.error("عجزنا نوصل للذكاء الاصطناعي حالياً، جربي تضغطين الزر مرة ثانية.")
                else:
